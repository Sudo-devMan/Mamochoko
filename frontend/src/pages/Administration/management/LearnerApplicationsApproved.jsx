...
function LearnerApplicationsApproved() {
  ...
  return (
    <div>
        ...
        <div className='m-3'>
            <h4 className='text-dark fs-3'>Approved Applications</h4>
            <Link className='btn btn-danger' to={'/admin/learner-applications'}>Go to rejected applications</Link>
        </div>
        ...
        <div className='p-lg-3 p-1'>
            ...
            <hr />
            <h3 className="text-success">Approved ({applications.length})</h3>
            {
                    ...
                    return (
                        <div key={i} className='card col-lg-3 d-inline-block m-2'>
                            ...
                            <div className="card-body">
                                ...
                                <p><span className="fw-bold">Applied for: </span>Grade {a.grade_applied_for}</p>
                                <p><span className="fw-bold">Date applied: </span>{fixDate(a.date_applied)}</p>
                                <p><span className="fw-bold">Status: </span>{!a.rejected ? <span className="text-success fw-bold">APPROVED</span> : <span className='text-danger fw-bold'>Rejected</span>}</p>
                                <p><span className="fw-bold">Approved by: </span>{a.approved_by.username ? a.approved_by.username :'sumn'}</p>
                                <p><span className="fw-bold">Reason for approval: </span>{a && a.approved_reason.slice(0, 10)}...</p>
                            </div>
                            <div className="card-footer">
                                <Link to={`/admin/learner-applications/${a.id}/detail`} className="card-link btn btn-primary">See In Detail</Link>
                            </div>
                ...
                : loading ? <h1 className='m-5 text-muted text-center'>LOADING...</h1> : <h1 className='m-5 text-muted text-center'>NO <span className="text-success">APPROVED</span> APLICATIONS</h1>
            }
        </div>
    </div>
  )
...
export default LearnerApplicationsApproved;